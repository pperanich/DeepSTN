...
from DATA.lzq_read_data_time_poi import lzq_load_data, prepare_data
...
if XDST:
    ...
    print("loading data...")
    X_train, T_train, P_train, Y_train, X_test, T_test, P_test, Y_test, MM = prepare_data(all_data, poi, len_test,
...
if X11:
    ...
    for iterate_index in range(iterate_num):
        ...
        file_conv = os.path.dirname(os.path.realpath(__file__)) + '/DeepSTN_11/MODEL/DeepSTN_11_model_' + str(
            iterate) + '.hdf5'
        # train conv_model
        if train11:
            model_checkpoint = ModelCheckpoint(
...
if X10:
    ...
    for iterate_index in range(iterate_num):
        ...
        file_conv = os.path.dirname(os.path.realpath(__file__)) + '/DeepSTN_10/MODEL/DeepSTN_10_model_' + str(
            iterate) + '.hdf5'
        # train conv_model
        if train10:
            model_checkpoint = ModelCheckpoint(
...
if X01:
    ...
    for iterate_index in range(iterate_num):
        ...
        file_conv = os.path.dirname(os.path.realpath(__file__)) + '/DeepSTN_01/MODEL/DeepSTN_01_model_' + str(
            iterate) + '.hdf5'
        # train conv_model
        if train01:
            model_checkpoint = ModelCheckpoint(
...
if X00:
    ...
    for iterate_index in range(iterate_num):
        ...
        file_conv = os.path.dirname(os.path.realpath(__file__)) + '/DeepSTN_00/MODEL/DeepSTN_00_model_' + str(
            iterate) + '.hdf5'
        # train conv_model
        if train00:
            model_checkpoint = ModelCheckpoint(
...
X_train, T_train, P_train, Y_train, X_test, T_test, P_test, Y_test, MM = prepare_data(all_data, poi, len_test,
                                                                                      len_closeness, len_period,
                                                                                      len_trend, T_closeness, T_period,
                                                                                      T_trend)
